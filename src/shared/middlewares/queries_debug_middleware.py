...
class QueriesDebugMiddleware(object):
    ...
    def process_response(self, request, response):
        ...
        if response.status_code in [200, 201, 204]:
            ...
            Metrics.number_queries.labels(view_name, request.method).inc(len(connection.queries))
            logger.debug('%s: %s queries run, total %s seconds' % (view_name, len(connection.queries), round(total_response_time, 4)))
        return response
